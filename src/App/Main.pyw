...
class CTD_Thread(QObject):
    ...
    def __init__(self, ser):
        ...
        self.working = True
        self.ser = serial.Serial(port=ser.port, baudrate=ser.baudrate, timeout=5)
        _Config = Cfg()
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.setWindowFlags(QtCore.Qt.WindowCloseButtonHint | QtCore.Qt.WindowMinimizeButtonHint | QtCore.Qt.WindowStaysOnTopHint)                            # Pone la ventana en modo persistente
        self.setupUi(self)
...
#        self.setMouseTracking(True)
        ...
        self.txt_EstGral.textChanged.connect(self.chg_txt_EstGral)          #Se genera al cambiar el contenido del cuadro
        self.txt_fTSG.editingFinished.connect(self.chg_txt_fTSG)            #Se generea el evento cuando pierdo focus o se presiona enter
    ...
    def showTime(self):
        ...
        if self.flag:
            ...
            self.setWindowTitle('Planilla V.%s DEMO (%.0f)' %(__version__, self.count))
    ...
    def init_CTD(self):
        try:
            ...
            ser.close()
            self.CTD = CTD_Thread(ser)                                       # a new worker to perform those tasks
            self.thread_CTD = QThread()                                    # a new thread to run our background tasks in
            self.CTD.moveToThread(self.thread_CTD)                        # move the worker into the thread, do this first before connecting the signals
            self.thread_CTD.started.connect(self.CTD.work)                # begin our worker object's loop when the thread starts running
            self.CTD.intReady.connect(self.onIntReadyCTD)
            self.CTD.finished.connect(self.loop_finished)                  # do something in the gui when the worker loop ends
            self.CTD.finished.connect(self.thread_CTD.quit)               # tell the thread it's time to stop running
            self.CTD.finished.connect(self.CTD.deleteLater)               # have worker mark itself for deletion
            self.thread_CTD.finished.connect(self.thread_CTD.deleteLater)  # have thread mark itself for deletion
        except IOError:
            self.loop_finished()                                # Vuelen boton iniciar y detener a condiciones iniciales
            self.msg_Box(mensaje = 'Error al inicializar el puerto seleccionado!!!\r', titulo = 'I/O Error CTD', icono = QMessageBox.Critical)
            return
    ...
    def init_TSG(self):
        try:
            ...
            ser.close()
            self.TSG = TSG_Thread(ser)                                      # a new worker to perform those tasks
            self.thread_TSG = QThread()                                     # a new thread to run our background tasks in
            self.TSG.moveToThread(self.thread_TSG)                          # move the worker into the thread, do this first before connecting the signals
            self.thread_TSG.started.connect(self.TSG.work)                  # begin our worker object's loop when the thread starts running
            self.TSG.intReady.connect(self.onIntReadyTSG)
            self.TSG.finished.connect(self.loop_finished)                   # do something in the gui when the worker loop ends
            self.TSG.finished.connect(self.thread_TSG.quit)                 # tell the thread it's time to stop running
            self.TSG.finished.connect(self.TSG.deleteLater)                 # have worker mark itself for deletion
            self.thread_TSG.finished.connect(self.thread_TSG.deleteLater)   # have thread mark itself for deletion
        except IOError:
            self.loop_finished()                                            # Vuelen boton iniciar y detener a condiciones iniciales
    ...
    def getSkipover(self):
        self.estacion[self.txt_EstGral.text()]['Skipover'] = self.CTD_str['Scan']
        self.save_json()
        self.btn_skipover.setEnabled(False)
    ...
    def click_btn_Exportar(self):
        ...
        R_File = open(self.W_Dir + os.sep + 'Est%s.txt' %self.txt_EstGral.text().zfill(4), "r")
        dato = R_File.readlines()
        R_File.close()
        planilla['A2'] = self.cfg['Campaña']['SiglasBuque'].upper()+self.cfg['Campaña']['Año']+str(int(self.cfg['Campaña']['NroCampaña'])).zfill(2)
        flag_nmea = True
        flag_cub = True
        flag_sup = True
        for i in dato:
        ...
        xls.save(self.W_Dir + os.sep + '%s.xlsx' %self.txt_EstGral.text().zfill(4))
    ...
    def Frm_Note_load(self):
        ...
        self.win_note.lbl_Titulo.setText(f"Estación General {self.txt_EstGral.text()}")
        self.win_note.exec_()
        self.estacion[self.txt_EstGral.text()]['Comentarios'] = self.win_note.comentario
        self.save_json()
    ...
    def W_Pos(self):
        if self.statusAdq:
            ...
            self.estacion[self.txt_EstGral.text()]['Posicion']['Inicio']['Latitud'] = self.NMEA.line['latD']
            self.estacion[self.txt_EstGral.text()]['Posicion']['Inicio']['Longitud'] = self.NMEA.line['lonD']
            self.estacion[self.txt_EstGral.text()]['FechaHora']['Inicio']['HoraGMT'] = self.NMEA.line['hora']
            self.estacion[self.txt_EstGral.text()]['FechaHora']['Inicio']['FechaGMT'] = self.NMEA.line['fecha']
            self.estacion[self.txt_EstGral.text()]['Batimetria']['Inicio']=1
        else:
            # Deje de adquirir. Modifico las variables finales
            self.estacion[self.txt_EstGral.text()]['Posicion']['Fin']['Latitud'] = self.NMEA_Str['latD']
            self.estacion[self.txt_EstGral.text()]['Posicion']['Fin']['Longitud'] = self.NMEA_Str['lonD']
            self.estacion[self.txt_EstGral.text()]['FechaHora']['Fin']['HoraGMT'] = self.NMEA_Str['hora']
            self.estacion[self.txt_EstGral.text()]['FechaHora']['Fin']['FechaGMT'] = self.NMEA_Str['fecha']
            self.estacion[self.txt_EstGral.text()]['Batimetria']['Fin']=10 
        self.save_json()      

    def W_Bott(self):
        try:
            self.estacion[self.txt_EstGral.text()]['Botellas'][self.CTD_str['Bot']] = self.CTD_str
        except:
            pass
        self.save_json()
    ...
    def W_TSGvsCTD(self):
        ...
        self.estacion[self.txt_EstGral.text()]['TSG_file'] = self.txt_fTSG.text()
...
class env():
    ...
    def load_env(self):
        ...
        powershell_command = f"{os.getcwd()}{os.sep}SIAVO_Cfg.exe -f {self.env} --mkroot -e"              # Creo la estructura de datos a partir del archivo env"
        result = subprocess.run(["powershell", "-Command", powershell_command], capture_output=True, text=True, shell=True)
    ...
    def __crear_estructura(self):
        try:
            ...
            logging.error(f"No se pudo crear la estructura de datos. No se encontro el archivo de variables de entorno {self.paths}")        
    ...
    def __CrearCamp(self):
        try:
            ...
            for i, linea in enumerate(lines):
                if linea.startswith(f'# Estructura datos oceanograficos'):
                    root = os.getenv('OF_DATA_ROOT')            # Ruta principal
                    path =  os.getenv('OF_BUQUE_NROCAMP')       # Ruta de la campaña activa
                                    ...
                                    if key == "termosal":                                 
                                        for tsg_key, tsg_item in item.items(): 
                                            ...
                                            try:
                                                ...
                                                logging.error(f"No se pudo crear el directorio {path_tsg}")                                                                       
                                    else:
                                        try:
                                            path_ctd = f"{path}{os.sep}{item}" 
                                            os.makedirs(f"{path_ctd}") 
                                        except:
                                            logging.error(f"No se pudo crear el directorio {path_ctd}")

                                logging.info(f"Estructura de campaña generada con exito. {path}")
                                # Copio el archivo json en la estructura de la campaña
                                shutil.copy(self.f_json, os.getenv('OF_BUQUE_NROCAMP'))
                        else:                  
                            logging.critical('Debe crear la estructura de datos antes de incorporar una campaña.')
                            sys.exit(0)
